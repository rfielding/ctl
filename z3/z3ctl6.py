...
class BusinessSystemSimulator:
    ...
    def generate_mermaid_state_diagram(self, process_id: str) -> str:
        ...
        mermaid.append(f"    title: {process['description']}")
        mermaid.append("")
        
        # Define states
        for state in process['states']:
            clean_state = state.replace("_", " ").title()
            mermaid.append(f"    {state} : {clean_state}")
            ...
            for transition_key, count in self.counters.transition_types.items():
                if transition_key.startswith(f"{process_id}:"):
                    ...
                    if "->" in transition:
                        ...
                        mermaid.append(f"    {from_state} --> {to_state} : {count} transitions")
        else:
            # Default transitions between consecutive states
            states = process['states']
            for i in range(len(states) - 1):
                mermaid.append(f"    {states[i]} --> {states[i+1]}")
            
            # Add loop back to initial state
            if len(states) > 1:
                mermaid.append(f"    {states[-1]} --> {process['initial_state']}")
    ...
    def generate_mermaid_sequence_diagram(self) -> str:
        ...
        mermaid.append("    title: Business Process Message Flow")
        mermaid.append("")
        
        # Participants (processes)
        participants = list(self.processes.keys())
        for participant in participants:
            role = self.processes[participant].get('business_role', participant)
            mermaid.append(f"    participant {participant} as {role}")
        
        mermaid.append("")
        
        # Create a simplified interaction flow based on channels
        interactions = [
            ("Customer", "order_service", "Place Order"),
            ("order_service", "payment_service", "Process Payment", "payment_requests"),
            ("payment_service", "order_service", "Payment Confirmed", "order_events"),
            ("order_service", "inventory_service", "Check Stock", "inventory_queries"),
            ("inventory_service", "order_service", "Stock Available", "order_events"),
            ("order_service", "fulfillment_service", "Fulfill Order", "order_events"),
            ("fulfillment_service", "order_service", "Order Shipped", "order_events"),
            ("order_service", "Customer", "Shipping Notification")
        ]
        
        for interaction in interactions:
            if len(interaction) == 3:
                from_actor, to_actor, message = interaction
                mermaid.append(f"    {from_actor}->>+{to_actor}: {message}")
            elif len(interaction) == 4:
                from_actor, to_actor, message, channel = interaction
                # Add channel info from counters if available
                if hasattr(self, 'counters') and channel in self.counters.messages_sent:
                    count = self.counters.messages_sent[channel]
                    mermaid.append(f"    {from_actor}->>+{to_actor}: {message} ({count} msgs)")
                else:
                    mermaid.append(f"    {from_actor}->>+{to_actor}: {message}")
        
        # Add error flows
        mermaid.append("")
        mermaid.append("    Note over payment_service: 5% payment failures")
        mermaid.append("    Note over inventory_service: 2% out of stock")
        mermaid.append("    Note over order_service: 10% validation failures")
        
        mermaid.append("```")
        return "\n".join(mermaid)
    
    def generate_mermaid_metrics_chart(self, metric_type: str = "throughput") -> str:
        """Generate Mermaid chart for simulation metrics"""
        mermaid = []
        
        if metric_type == "throughput":
            mermaid.append("```mermaid")
            mermaid.append("xychart-beta")
            mermaid.append("    title \"Message Throughput by Channel\"")
            mermaid.append("    x-axis [\"Payment Requests\", \"Order Events\", \"Inventory Queries\"]")
            mermaid.append("    y-axis \"Messages\" 0 --> 50")
            
            # Get data from counters
            if hasattr(self, 'counters'):
                payment_msgs = self.counters.messages_sent.get('payment_requests', 0)
                order_msgs = self.counters.messages_sent.get('order_events', 0)
                inventory_msgs = self.counters.messages_sent.get('inventory_queries', 0)
                
                mermaid.append(f"    bar [{payment_msgs}, {order_msgs}, {inventory_msgs}]")
            else:
                mermaid.append("    bar [10, 25, 15]")
            
            mermaid.append("```")
        
        elif metric_type == "business_events":
            mermaid.append("```mermaid")
            mermaid.append("pie title Business Events Distribution")
            
            if hasattr(self, 'counters'):
                total_events = sum(self.counters.business_events.values())
                if total_events > 0:
                    for event, count in self.counters.business_events.items():
                        percentage = (count / total_events) * 100
                        clean_event = event.replace("_", " ").title()
                        mermaid.append(f"    \"{clean_event}\" : {percentage:.1f}")
                else:
                    mermaid.append("    \"No Events\" : 100")
            else:
                mermaid.append("    \"Orders Received\" : 35")
                mermaid.append("    \"Payments Processed\" : 30")
                mermaid.append("    \"Items Shipped\" : 25")
                mermaid.append("    \"Errors\" : 10")
            ...
            mermaid.append("```mermaid")
            mermaid.append("xychart-beta")
            mermaid.append("    title \"Process Performance Metrics\"")
            mermaid.append("    x-axis [\"Order Processing\", \"Payment\", \"Inventory\", \"Fulfillment\"]")
            mermaid.append("    y-axis \"Average Latency (seconds)\" 0 --> 5")
            
            # Sample performance data
            if hasattr(self, 'counters'):
                # Calculate average latencies by operation type
                order_latency = self._get_avg_latency_for_process("order_service")
                payment_latency = self._get_avg_latency_for_process("payment_service") 
                inventory_latency = self._get_avg_latency_for_process("inventory_service")
                fulfillment_latency = self._get_avg_latency_for_process("fulfillment_service")
                
                mermaid.append(f"    line [{order_latency:.2f}, {payment_latency:.2f}, {inventory_latency:.2f}, {fulfillment_latency:.2f}]")
            else:
                mermaid.append("    line [0.8, 2.1, 0.3, 12.5]")
...
if __name__ == "__main__":
    # First show the Mermaid diagram types
    example_mermaid_showcase()
    
    print("\n" + "="*80)
    print("FULL VIBECODE BUSINESS SYSTEM WITH MERMAID DIAGRAMS")
    print("="*80)
    
    # Run the full vibecode business system example
